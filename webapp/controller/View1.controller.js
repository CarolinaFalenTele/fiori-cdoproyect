sap.ui.define([
     ...
     */
    function (Controller,VizFrame, MessageToast , JSONModel, UIComponent,FlattenedDataset, Popover , FeedItem ,DateFormat) {
        "use strict";
             ...
             onSubmit: function() {
                ...
                aPanelIds.forEach(function(sPanelId) {
                    ...
                    if (oFormPanel) {
                        ...
                        var aFields = oFormPanel.findElements(true /* recurse */)
                            .filter(function(oElement) {
                                return oElement instanceof sap.m.Input || oElement instanceof sap.m.Select || oElement instanceof sap.m.DatePicker; // Filtrar controles de entrada, selección y fecha
                            });
                        ...
                        var bAllFieldsFilled = aFields.every(function(oField) {
                            ...
                            if (oField instanceof sap.m.Input) {
                                ...
                                sValue = oField.getDateValue() ? oField.getDateValue().toISOString() : ""; // Convertir la fecha a un formato de cadena
                            }
                            if (!sValue) {
                ...
                if (bAllPanelsCompleted) {
                  ...
                  MessageToast.show("Por favor, completa todos los campos de todos los paneles.");
        ...
        //-------------- Get Inputs -------------------------------------------------------
            onInputChange: function() {
                ...
                var sSelectValue = oSelect.getSelectedItem() ? oSelect.getSelectedItem().getText() : "";
                var oSelect1 = this.byId("slct_Jefe");
                var sSelectValue1 = oSelect1.getSelectedItem() ? oSelect1.getSelectedItem().getText() : "";
                var oSelect2= this.byId("slct_area");
                var sSelectValue2 = oSelect2.getSelectedItem() ? oSelect2.getSelectedItem().getText() : "";
                var oSelect3= this.byId("slct_clFun");
                var sSelectValue3 = oSelect3.getSelectedItem() ? oSelect3.getSelectedItem().getText() : "";
                ...
                var sFormattedDateIni = sStartDatePicker ? DateFormat.getDateInstance({ pattern: "yyyy-MM-dd" }).format(sStartDatePicker) : "";

                var oDatePicker = this.byId("date_fin");
                var sDateValue = oDatePicker.getDateValue();
                var sFormattedDate = sDateValue ? DateFormat.getDateInstance({ pattern: "yyyy-MM-dd" }).format(sDateValue) : "";
        ...
        handleInformation: function(oEvent) {
            if (!this.newInformationDialog) {
                this.newInformationDialog = sap.ui.xmlfragment("solicitudproyect.view.ViewInfo", this);
